...
# 답: 1) ( 비즈니스 로직 처리 ) / 2) ( db 와 Template 연결 )
...
│   ├── apps.py
│   ├── models.py      <-- [DB설정] ServiceLog 데이터 모델 정의 ( Jpa Entity )
...
```python
# DB에 생설할 데이터구조를 Class로 작성 (JPA Entity 선언과 동일)
...
```bash
# 1. 마이그레이션 생성 - models.py의 변경사항을 db 설계도를 생성
...
# 답: 1) python manage.py ( makemigration ) 
...
## M(model) T(html,css:화면) V(Controller: 비지니스로직, db와 view와 연결)
...
def dashboard_view(request):
    ...
    if qs.exists():
        ...
        #                그룹핑        합계     재정렬
        summary_df = df.groupby()[[]].sum().reset_index()
        # 3. 템플릿 전달용 순수 파이썬 리스트 추출
        categories = summary_df['category'].tolist()
        visitors = summary_df['visitors'].tolist()
        sales = summary_df['sales'].tolist()
    else:
        categories, visitors, sales = [],[],[]
    # 4. 템플릿(html)전달할때 바인딩객체
    context = {
