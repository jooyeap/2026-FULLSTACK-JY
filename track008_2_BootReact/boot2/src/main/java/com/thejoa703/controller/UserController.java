...
public class UserController {
	...
	public ResponseEntity<UserResponseDto> createUser(@RequestBody UserRequestDto requestDto){
		UserResponseDto response = userSerivce.createUser(requestDto);
		return ResponseEntity.status(HttpStatus.CREATED).body(response); // HttpStatus.CREATED 201
	}
	
	// 사용자 단건 조회 - /api/users/1 해당번호
	@Operation( summary = "사용자 단건 조회", description = "사용자 ID로 특정 회원 정보를 조회합니다.")
	@GetMapping("/{id}")
	public ResponseEntity<UserResponseDto> getUser(@PathVariable("id") Long id){
		UserResponseDto response = userSerivce.getUser(id);
		return ResponseEntity.ok(response); // ok 200
		//return ResponseEntity.status(HttpStatus.OK).body(response); 위와 동일 
	}
