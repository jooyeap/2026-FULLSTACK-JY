...
function SignupPage(){
    ...
    // 데이터 받아서 회원가입 전송 - 네트워크가 느리면 2~3회 연속으로 클릭 (회원가입요청중복)
                ...
                rules={[ 
                    ...
                    {validator: async(_, value) => {
                        ...
                        try {
                            ...
                            if(res?.data === true) {
                                return Promise.reject(new Error("이미 사용중인 닉네임 입니다."));
                            }
                            return Promise.resolve();
                        } catch (err) {
                            console.log('닉네임 중복 검사 오류', err);
                            return Promise.reject(new Error("중복 검사 실패"));
                        }
                    }},
                ]}
