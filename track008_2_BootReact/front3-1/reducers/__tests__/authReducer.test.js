...
describe('user slice reducer', () => {
    ...
    it('signupRequest', () => {
        ...
        // 2. 리듀서 툴킷에서 { type: signupRequest, payload: userdata } 객체 생성
        // 3. 리듀서의 signupRequest: (state,action) => {} 실행
        //    action = { type:signupRequest, payload:userData }
    ...
    it('signupFailure', () => {
        ...
        // 1. signupSuccess('회원가입 실패') 실행하면
        // 2. 리듀서 툴킷에서 { type: signupFailure, payload: '회원가입 실패' } 객체 생성
        // 3. 리듀서의 signupFailure: (state,action) => {} 실행
        //    action = { type:signupFailure, payload:'회원가입 실패'}
        expect(state.loading).toBe(false);
        expect(state.error).toEqual('회원가입 실패'); // state.error = action.payload
        expect(state.success).toBe(false);
    });
    ...
    it('resetUserState', () => {
        ...
        // 3. 리듀서의 resetUserState : (state,action) => {} 액션 받아서 처리
        //    action = { type:resetUserState, payload:userData }
        expect(state.loading).toBe(false);
        expect(state.error).toBeNull();
        expect(state.success).toBe(false);
    });
