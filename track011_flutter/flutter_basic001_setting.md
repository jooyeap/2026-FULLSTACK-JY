...
```bash
# Flutter 환경 점검 명령어 
# Android SDK, Vs Code, C++ 빌드도구 등 사용 가능한지 점검 (!, x)
flutter doctor
...
# 답: flutter doctor --( flutter doctor --android -licenses )

# Q2. 현재 내 PC에 연결되어 앱을 구동할 수 있는 디바이스 리스트를 조회하는 명령어는 무엇인가요?
# 답: flutter ( flutter devices )
...
void main() {
  ...
  Person p = Person("홍길동", 22);
  p.sayHello();

  // 8. 비동기 처리 (Future : 미래에 완료될 작업, async/await : 작업할때 까지 기다리기, then : 작업 완료 후)
  fetchData().then( (value) => print("then (1): $value") ); // #1 fetchData() 2초 타이머 / #2 then으로 예약
  testAsync(); // #3 다른 2초 타이머
}
