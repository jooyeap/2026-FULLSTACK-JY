...
class BoardNotifier extends Notifier<BoardState> {
  ...
  void _initDio() {
    _dio = Dio(BaseOptions( // 기본옵션 설정
      //10.0.0.2 / localhost:8080 / https://thejoa703.ducksdns.org
    ...
    _dio.interceptors.add(InterceptorsWrapper( // ## InterceptorsWrapper
      ...
      onRequest: (options, handler) async {
        ...
        if (token != null) { // 토큰 유무 여부
          options.headers['Authorization'] = 'Bearer $token'; // http 헤더에 Bearer 토큰 추가
        }
...
import '../data/board_provider.dart'; // 전역상태 + 서버 연동데이터 가져오는 기능 (boardProvider)
...
class _PostWritePageState extends ConsumerState<PostWritePage> {
  ...
  Future<void> _pickImages() async {
    ...
    if (images.isNotEmpty) {
      for (var image in images) { // 선택된 파일들 
        final bytes = await image.readAsBytes(); // 바이트로 데이터 추출
  ...
  Widget build(BuildContext context) {
    return Scaffold( //# 앱의 기본골격
      appBar: AppBar(title: const Text('새 글 작성')), // 상단 앱 바
      body: Padding( // 여백 레이아웃
        padding: const EdgeInsets.all(16.0), // 4개(상하좌우) 여백
...
class _PostListPageState extends ConsumerState<PostListPage> {
  ...
  String _resolveImageUrl(String url) { // url 변환
    if (url.startsWith('http://') || url.startsWith('https://')) {
      return url;
    }
    final String serverBaseUrl = ApiClient.getBaseUrl(); // https://주소.ducksdns.org, localhost:8080
  ...
  @override
  Widget build(BuildContext context) {
    final boardState = ref.watch(boardProvider); // 지속적으로 확인 (useEffect)
                          ...
                          child: InkWell(
                            onTap: () {
                              Navigator.push( // 상세 페이지로 이동
                                ...
                                MaterialPageRoute(
                                  builder: (context) =>
                                      PostDetailPage(post: post), // 상세 페이지 위젯
                                ),
                              );
                            },
                            child: Padding(
...
```bash
flutter pub get
dart run flutter_launcher_icons # 아이콘 생성 스크립트
