...
// ConsumerWidget 상속 받으면 React에서 useSelector 처럼 전역인증
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
...
// AuthState - Redux
// _dio.interceptors : api 요청마다 `Barear ${token}` 을 헤더주입, 401 에러토큰

// 리덕스
import 'package:flutter_riverpod/flutter_riverpod.dart';
// axios 역할의 비동기 http 통신
...
class AuthNotifier extends Notifier<AuthState> {
  @override
  AuthState build() {
    _initDio(); // Provider 생성시 인터셉터 초기화
    return const AuthState(); // 초기화 상태 반환
  }

  late final Dio _dio; // late (나중에, 사용하기 직전에), final 변경 x 
  // OS 암호화 저장소 객체 생성 (localStorage 대신 모바일 보안 영역 사용)
  final _storage = const FlutterSecureStorage(); // os 암호화 로컬 저장소 생성
  ...
  void _initDio() {
    ...
    _dio.interceptors.add(InterceptorsWrapper(
      // 매 api 요청마다 SecureStorage에서 토큰 읽어와서 Authorization 에서 주입
  ...
  Future<bool> _refreshAccessToken() async {
    try {
      final response = await _dio.post('/auth/refresh'); // boot 요청
  ...
  Future<bool> checkNicknameDuplicate(String nickname) async {
    try {
      ...
      return response.data; // true : 이미 존재(중복되는 값 있음) / false : 사용가능 (중복 X)
    } catch (e) {
      return false;
    }
  }
...
class LoginPage extends ConsumerStateWidget {
