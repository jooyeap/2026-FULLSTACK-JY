 ...
 *   post:
 *     summary: 로그인 (테스트용)
 *     description: Passport 이메일과 비밀번호를 인증해서 세션 저장
 *     requestBody: 
 *       required: true
 *       content:
 *         application/json:
 ...
 *   get:
 *     summary: 전체 사용자 조회
 *     description: 로그인 된 전체 사용자 목록을 조회할 수 있습니다.
 *     responses:
 *       200:
 ...
 *     responses:
 *       200:
 *         description: 닉네임 수정 성공 (테스트용)
 *       401:
 *         description: 로그인 후 사용
 */
router.patch('/:appUserId/nickname', isAuthenticated, async(req, res)=>{
    try {
        const {nickname, appUserId} = req.body;
        await updateUserNickname(nickname , req.params.appUserId);
 ...
 *     responses:
 *       200:
 *         description: 사용자 삭제 성공 (테스트용)
 *       401:
 *         description: 로그인 후 사용
 */
router.delete('/:appUserId', isAuthenticated, async(req, res)=>{
    try {
        await deleteUser(req.params.appUserId);
