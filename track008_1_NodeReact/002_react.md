...

{
  ...
  "license": "ISC",
  "dependencies": {
    "@ant-design/icons": "^4.3.0", // ant disign UI 라이브러리
    "@next/bundle-analyzer": "^12.3.4", // 사용하는 용량 - 시각분석도구
    "@reduxjs/toolkit": "^1.9.7", // 전역 저장공간
    "antd": "^4.8.6",
    "axios": "^1.3.4", // 백엔드 api 서버와 통신 http 라이브러리
    "babel-plugin-styled-components": "^1.12.0", // ssr환경 클래스명 충돌 방지
    "bootstrap": "^5.3.8", // 전반적인 웹 스타일링
    "cross-env": "^7.0.2", // 운영체제 상관없이 환경변수 설정
    "dotenv": "^16.0.1", // 중요 키
    "express": "^5.2.1", // 웹 서버
    "faker": "^4.1.0",  // 가짜 데이터 무작위 생성 도구
    ...
    "prop-types": "^15.7.2", // 전달되는 props타입 검사 도구
    "react": "^17.0.2", 
    ...
    "styled-components": "^5.2.1", // css 작성 가능하게
    "swr": "^0.3.9"
  },
  "devDependencies": {
...
2. 설정 파일
front/
├── .babelrc      # Babel 설정 파일 : jsx -> 일반 js로 변환해서 브라우저가 이해할수 있게
...
2) [pages]      -   _app.js 모든 페이지 들을 `Layout`으로 감싸는 진입점
3) [pages]      -  index.js 본문 컨텐츠 구동 시작점

> _app.js Layout을 감싸서 영역 안에 {children} 
step1) localhost:3000 접속
step2) Layout에서의 헤더, 네비게이션, 푸터 렌더링
step3) 본문 영역의 main.container 안에 컨텐츠 출력
...
```
front/
└── pages/                  # ✅ Redux 스토어 설정 폴더
│   ├── _app.js             # Next.js 기반 페이지 설정
│   ├── index.js            # 메인 페이지
│   ├── login.js            # 로그인 페이지
│   ├── signup.js           # 회원가입 페이지
│   └── user.js             # 사용자 목록, 정보페이지           
```
...
2) `useEffect` -> 로그인 여부 확인 및 사용자 목록 불러오기
  - 생명주기 및 부수효과처리
  - 부품 맨 처음 나올때, 특정상태 변경시
3) `dispatch` -> 액션 발생 (로그인, 사용자 삭제,,,)
  - 알림 : 스토어한테 이러한 액션 발생 알려주는 수단
