...
import {
    login, signUp, logout, loadUsers, updateNickname, deleteUser,
    loginApi, signUpApi, logoutApi, loadUsersApi, updateNicknameApi, deleteUserApi,
} from './user'; // http요청 라이브러리
import reducer, {
    initialState,
...
describe('user saga', () => {
    ...
    it( 'login success', () => {
        const action = {type: LOG_IN_REQUEST, data:{ email:'z@z', password:'z', nickname:'zzz'}};
        const gen = login(action);

        expect(gen.next().value).toEqual(call(loginApi, action.data));
        const apiResponse = {APP_USER_ID:1, EMAIL: '1@1', NICKNAME:'first'};
        expect(gen.next({ data:apiResponse })
                  .value).toEqual( put( { type: LOG_IN_SUCCESS,
                                          data: {id:1, email:'1@1', nickname:'first'}
                   } ) );

        const error = {response: { data : '로그인 실패' }};
        expect(gen.throw(error).value)
                  .toEqual( put( { type: LOG_IN_FAILURE, error:'로그인 실패'} ) );
    });
    ...
    it( 'logout success', () => {
        ...
        expect(gen.next().value).toEqual(call(logoutApi, action.data));
    ...
    it( 'loadUsers success', () => {
        ...
        const fakeUsers = [{APP_USER_ID:1, EMAIL: '1@1', NICKNAME: 'first'}]
        expect(gen.next({ data:fakeUsers }).value)
                  .toEqual( put( { type: LOAD_USER_SUCCESS,
                                   data: [{id:1, email:'1@1', nickname:'first'}]
                   } ) );

        const error = {response: { data : '유저로드 실패' }};
        expect(gen.throw(error).value)
                  .toEqual( put( { type: LOAD_USER_FAILURE, error:'유저로드 실패'} ) );
    });

    it( 'updateNickname success', () => {
        const action = {type: UPDATE_NICKNAME_REQUEST, data:{id:1, nickname:'test'}};
