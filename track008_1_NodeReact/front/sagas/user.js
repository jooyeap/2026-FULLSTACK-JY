/*
    ...
    - axios로 API를 호출 -> 성공/실패에 따라서 reducer로 액션 전달
    -
*/
...
export function* login(action){
    try{
        ...
        // return { ...state, isLoading: false, error:action.error?.message || action.err}
    }
}

function* watchLogin(){
    yield takeLatest( LOG_IN_REQUEST, login );
    // SIGN_UP_REQUEST 액션 발생 -> 여러번 요청시 가장 마지막 요청처리 1개
}

// - 로그아웃 watchLogout
// post : /user/logout (requestBody)
export function logoutApi(){
    return client.post('/user/logout'); // http://localhost:3065/user/logout
}
export function* logout(){
    try{
        yield call ( logoutApi ); // api 호출
...
export function* loadUsers(){
    try{
        ...
        yield put ( {type:LOAD_USER_FAILURE, error: err.response?.data || err.message} );
        // return { ...state, isLoading: false, error:action.error?.message || action.err}
    }
}
...
export function updateNicknameApi(data){
    return client.patch(`/user/${data.id}/nickname`, {nickname:data.nickname}); // http://localhost:3065/user/
}
export function* updateNickname(action){
    try{
        const result = yield call ( updateNicknameApi, action.data); // api 호출, action.data: email, password
...
export function deleteUserApi(id){
    return client.delete(`/user/${id}`); // http://localhost:3065/user/
}
export function* deleteUser(action){
    try{
        const result = yield call ( deleteUserApi, action.data.id ); // api 호출, action.data: email, password
