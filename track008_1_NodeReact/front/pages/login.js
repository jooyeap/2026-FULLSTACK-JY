...
export default function LoginPage(){
    ...
    // form 연결
    const [email, setEmail] = useState(''); // email 변수, email 셋팅 함수
    const [password, setPassword] = useState('');
    
    // 1-1. 회원가입 했다면 알림창 - http://localhost:3000/login?signUpSuccess=true
    useEffect( () => {
        if(router.query.signUpSuccess == 'true'){
            alert('회원가입이 완료되었습니다. 로그인 해주세요.');
            // 알림창 띄우고, 쿼리제거(주소창)/ 새로고침해도 알림창이 다시 안뜨게
            // 주소표시창줄 , 주소표시창 줄 감출필요 없음, 새로고침해도 알림창 안뜨게
    ...
    // 1-3. 로그인후 사용자 목록 페이지로 이동
    useEffect( () => {
        // replace : 주소 표시창줄 바뀜, history 추가x
        // push : 주소 표시창줄 바뀜, history 추가o
