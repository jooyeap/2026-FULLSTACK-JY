...
abstract class Astronaut {
    ...
    abstract void rest();
    void condition() {System.out.println(this.name + "는 충분한 체력을 유지 중입니다.");}
}
...
public class AbstractEx002 {
    public static void main(String[] args) {
        // Astronaut astro = new Astronaut();  // Q1. 왜 객체 생성이 불가능한가? 추상화클래스 이므로 구현부가 없어 실체화 불가능
        ...
        for( Astronaut a : crew ) {
        	a.name = names[count];
        	a.stamina = stamina[count];
        	a.explore();
        	a.report();
        	// Q2. 체력이 낮은 대원만 휴식  70미만이 휴식을 취하게 만들기 
        	if(a.stamina < 70) { a.rest();}
        	else {a.condition();}
        	count++;
        }
    }
