...
public class Day027 {
	public static void main(String[] args) { 
		...
		users.add(new UserInfo("아이먼맨", 50));
		users.add(new UserInfo("헐크", 40));
		users.add(new UserInfo("캡틴", 120));
		users.add(new UserInfo("캡틴", 120));
		
		for(int i=0; i<users.size(); i++) {
			UserInfo a = users.get(i);
//			System.out.println((i+1)+" "+(UserInfo)users.get(i));
			System.out.printf("%d %s %d\n",(i+1),a.getName(),a.getAge());
		}
		System.out.println();
		
		Set<UserInfo> users1 = new HashSet<>();
		// 기차 : List -> 추가 : add, 가져오기 : 향상된 for/Iterator, 갯수 : size, 삭제 : remove, 포함여부 : contains
		users1.add(new UserInfo("아이언맨", 50));
		users1.add(new UserInfo("헐크", 40));
		users1.add(new UserInfo("캡틴", 120));
		users1.add(new UserInfo("캡틴", 120));
		
		for(UserInfo a : users1) {
			System.out.println(++count +" "+ a.getName() +" "+ a.getAge());
		}
...
- 저장공간의 크기를 [##3.유동적?(동적배열)]으로 관리함.

	String [] arr = new String[100]; - 자료형고정, 갯수고정
	List<UserInfo> list = new ArrayList<>(); - 내가 원하는객체, 갯수 무한대
...
- 인터페이스를 통해서 틀이 잡혀 있는 방법으로 
  ...
  Set  : ##6.   주머니  인덱스여부 [순서 : x] ,  중복허용여부 [x]  
  				 [add, get(x) -> Iterator/향상된 for문 , size, remove, contains] 
  Map  : ##7.   사전   [ 키:값 ] - 쌍(Entry)        
  				 [push, get(key값), size, remove, contains] 
