...
public class LambdaEx003 {
    public static void main(String[] args) {
        ...
        ListPrinter lp1 = new ListPrinter() {
        	@Override public void printAll(List<String> list) { list.forEach(System.out::println); } 
        };
        lp1.printAll(foods);

        //람다식으로 동일한 기능 구현하기
        ListPrinter lp2 = list -> list.forEach(System.out::println);
        lp2.printAll(foods);
        ...
        // 문제 2: 리스트 정렬
        ListSorter a = new ListSorter() {
        	@Override public void sort(List<Integer> list) { Collections.sort(list); }
        };
        a.sort(numbers);
        System.out.println("익명객체 결과: " + numbers);

        numbers = new ArrayList<>(Arrays.asList(5, 2, 9, 1, 3));
        ListSorter b = list -> Collections.sort(list);
        b.sort(numbers);
        System.out.println("람다식 결과: " + numbers);

        numbers = new ArrayList<>(Arrays.asList(5, 2, 9, 1, 3));
        ListSorter c = Collections::sort;
        c.sort(numbers);
