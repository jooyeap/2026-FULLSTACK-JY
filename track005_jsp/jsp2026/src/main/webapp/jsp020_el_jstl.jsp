...
1. 출력목적 EL(Expression Language, 표현식 언어)
2. 자바코드 없이 값을 출력 jstl과 같이 사용
      </pre>
      <p> <% out.println("hello"); %> </p>
      <p> <%="hello" %> </p>
      <p> ${"hello"} ${1} ${3.14} ${'A'}</p>
      
      <p><a href="jsp020_result.jsp?name=bubu" class="btn btn-primary">PARAM - 데이터 넘기기</a></p>
      <% // 서블릿 - 비즈니스 로직
      	Java001 data = new Java001(10,20);
      	request.setAttribute("data", data);
      %>
      <p><%=request.getAttribute("data") %></p>
      <p>${data}</p>
      <p>${data.a}</p>
      <p>${data.b}</p> <!-- 객체명.변수명 - getter()가 있어야함 -->
...
3. jar 파일 다운로드 - [WEB-INF]-[lib]
	https://mvnrepository.com/artifact/javax.servlet/jstl/1.2?utm_source=copilot.com
      ...
      <c:set var="numbers" value="3,2,1"></c:set>
...
<div class="container card my-5">
	<h4 class="card-header my-2">1) if</h4>
	<pre>
문제 설명:
jstl if를 사용하여
"20은 30보다 작거나 같다" 라는 문장을 출력해보세요.
	...
	<c:if test="${i<=30 }">20은 30보다 작거나 같다</c:if>  
	
	<h4 class="card-header my-2">2) choose</h4>
	<pre>
문제 설명:
변수 level을 3으로 설정하고,
jstl choose문을 이용하여
...
3이면 "고급", 그 외에는 "알 수 없음"을 출력해보세요.
	...
	<h4 class="card-header my-2">3) forEach</h4>
