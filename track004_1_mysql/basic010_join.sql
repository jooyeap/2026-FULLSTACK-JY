...
use mbasic;
desc userinfo;
create table join_userinfo select * from userinfo where 1=2;
desc join_userinfo;
...
-- delete from join_userinfo where no>= 1;
...
-- delete from join_userban where no>2;
...
select a.no, a.name, a.age, b.ban
from join_userinfo a join join_userban b
on a.no=b.no and a.age >= 20 and a.age <= 40;
-- >>> (3) join  using
select a.no, a.name, a.age, b.ban
from join_userinfo a join join_userban b
using(no)
...
from join_userban a left outer join join_userinfo b
on a.no=b.no;

select b.no, b.name, a.ban, b.age
from join_userban a left join join_userinfo b
on a.no=b.no;

select a.no, a.name, b.ban, a.age
from join_userinfo a right join join_userban b -- 오른쪽 테이블 보장
on a.no=b.no;

select a.no, a.name, b.ban, a.age
from join_userinfo a left join join_userban b -- 왼쪽 테이블 보장
on a.no=b.no;
...
select a.no, a.name, a.ban, b.age
from join_userban a left join join_userinfo b
on a.no=b.no;
...
-- -- 이코드가 의미하는바는? emp테이블과 dept 테이블의 모든 경우의 수를 출력하고 empno 기준으로 오름차정렬
-- -- 
...
-- >> select 에서 어느테이블의 데이터를 가져올건지 명시하지않아 오류발생
...
where a.mgr=b.empno and a.sal between c.losal and c.hisal
order by b.empno;
-- +-------+--------+------+-----------+-----------+
...
desc emp;
-- select a.empno, a.ename, a.mgr, if(a.mgr is null, b.empno is null ,b.empno) `MRG_EMPNO`, if(a.mgr is null,b.ename is null,b.ename) `MRG_ENAME`
-- from emp a, emp b
-- where a.mgr = b.empno
-- order by a.empno;
...
-- 기준점을 오른쪽 테이블로 잡아라
select a.empno, a.ename, a.mgr, b.empno`MGR_EMPNO`, b.ename`MRG_ENAME`
...
-- -- = 등가조인을 이용하시오.
select a.deptno, b.dname, avg(a.sal)`AVG_SAL`, max(a.sal)`MAX_SAL`, min(a.sal)`MIN_SAL`, count(*)`CNT`
from emp a join dept b
on a.deptno = b.deptno
...
-- -- left join을 이용하시오.
select a.deptno, a.dname, b.empno, b.ename, b.job, b.sal
...
select c.deptno, c.dname, a.empno, a.ename, a.mgr, a.sal, a.deptno, d.losal, d.hisal, d.grade, b.empno`MGR_EMPNO`,b.ename`MGR_ENAME`
-- 테이블 여러개 join??
-- from emp a, emp b, dept c, salgrade d
from dept c
left join emp a on c.deptno = a.deptno
left join emp b on a.mgr = b.empno
left join salgrade d on a.sal between d.losal and d.hisal
-- 아래 조건문들 풀어서 on으로 
-- where a.deptno = c.deptno and a.mgr=b.empno and a.sal between d.losal and d.hisal
-- c.deptno = a.deptno / a.mgr=b.empno / a.sal between d.losal and d.hisal
...
select ename from emp union select empno from emp;
