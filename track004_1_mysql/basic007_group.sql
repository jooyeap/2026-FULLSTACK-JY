...
select ban, count(*) from group_userinfo; -- 6 ( 집계안된 일반) , 1 (집계)
-- 10:42:27	select ban, count(*) from group_userinfo LIMIT 0, 100
-- Error Code: 1140. In aggregated query without GROUP BY, expression #1 of SELECT list contains nonaggregated column 'mbasic.group_userinfo.ban'; this is incompatible with sql_mode=only_full_group_by	0.000 sec

-- 줄수가 안맞아서 오류가 난것
select ban from group_userinfo; -- 6줄
select count(*) from group_userinfo; -- 1줄

select ban, count(*) -- 반, 갯수
from group_userinfo
group by ban; -- 반별 (집계필드가 아닌 다른것들이 다 들어감)  
-- +------+----------+
-- | ban  | count(*) |
-- +------+----------+
-- | A    |        2 |
...
select ban, avg(kor)`국어평균` , avg(eng)`영어평균`, avg(math)`수학평균` from group_userinfo group by ban having avg(kor)>=89;
-- +------+----------+----------+----------+
-- | ban  | 국어평균 | 영어평균 | 수학평균 |
-- +------+----------+----------+----------+
-- | A    |  95.0000 |  96.0000 |  98.0000 |
...
from group_userinfo group by ban having avg(age)>=35;
-- +------+----------+----------+----------+----------+
-- | ban  | 나이총합 | 나이평균 | 최고나이 | 최저나이 |
-- +------+----------+----------+----------+----------+
-- | B    |       77 |  38.5000 |       55 |       22 |
...
--            평균나이의 내림차순으로 정렬하시오.           
select ban, sum(age)`나이총합`, avg(age)`나이평균`, max(age)`최고나이`, min(age)`최저나이`
from  group_userinfo group by ban having avg(age)>=35 order by avg(age) desc;
-- +------+----------+----------+----------+----------+
-- | ban  | 나이총합 | 나이평균 | 최고나이 | 최저나이 |
-- +------+----------+----------+----------+----------+
-- | C    |      110 |  55.0000 |       66 |       44 |
...
select ban, count(case when age < 20 then 1 end)`미성년자수` from group_userinfo group by ban;
-- select ban, count( case when | 20세 미만인 사람 | ban 1개 마다 카운트)
...
select * from group_userinfo;
select ban, count(case when sns='n'  then 1 end)`sns수신거부수` from group_userinfo group by ban;
-- +------+---------------+
-- | ban  | sns수신거부수 |
-- +------+---------------+
-- | A    |             1 |
...
select ban, sum((kor+eng+math)/3)`평균총점` from group_userinfo group by ban having sum((kor+eng+math)/3)>80;
-- +------+-----------+
-- | ban  | 평균총점  |
-- +------+-----------+
-- | A    | 192.6667  |
...
select ban , sum((kor+eng+math)/6) from group_userinfo group by ban order by sum((kor+eng+math)/6) desc limit 1;
-- +------+-----------+
-- | ban  | 평균점수  |
-- +------+-----------+
-- | A    | 96.3333   |
...
select ban,
count(case when sex='m' then 1 end)`남학생수`,
count(case when sex='f' then 1 end)`여학생수`
from group_userinfo group by ban;
...
--   avg(sal)은 집계함수로 where은 일반 필드, having은 집계 함수에서 사용하므로 행의 수가 달라서 오류남
--  집계함수를 조건으로 사용하려면 having으로 집계 이후에 조정하고
--  집계함수를 조건으로 사용하지 않으면 where 사용해도 됨
