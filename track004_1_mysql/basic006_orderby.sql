...
alter table select_userinfo modify no int not null primary key auto_increment; -- key,자동증가 추가
...
insert select_userinfo (no,name) values (7,'seven');
...
-- from (적용할 테이블명)
-- where (조건 / 필요없으면 제외)
-- order by (기준 필드명) [desc(내림차순),asc(오름차순)] 
...
-- select no, age / 적용할 영역
-- from select_userinfo / 적용할 테이블
-- where / 조건 없으니 생략
-- order by age/필드명 desc/내림차순
-- limit 3/출력 개수; 

select * from select_userinfo order by age desc limit 3;
select * from select_userinfo order by age desc limit 2;
select * from select_userinfo order by age desc limit 0,2; -- 어디서부터, 몇개 / 0번째 부터 , 2개
