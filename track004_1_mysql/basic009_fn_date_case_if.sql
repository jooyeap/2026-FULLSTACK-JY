...
-- 1. 시스템의 현재 시각 조회 
select now(); -- 2026-05-26 14:37:04
select current_time(); -- 14:37:46
...
select date_format('2026-05-26 14:37:04', "%Y-%m-%d");
...
-- now(), weekday(0=월, 6=일), date_format(날짜,"%Y-%m-%d %H:%i-%s"), date-add
...
delete from date_userinfo where no >=1;
...
select date from date_userinfo where date_format(date,"%m");
...
-- if(조건, 참, 거짓)
-- 1이다 , 1이 아니다
select no , if(no=1, '1이다', '1이아니다')`1 상태` from control;

-- 1보다 크다, 작다
select no , if(no>1, '1보다크다', '1보다 크지않다')`1작다, 크다` from control;
...
update if_userinfo set age=11 where no=1;
insert into if_userinfo (no, name, age, sex, sns) values (1,'first',11,null,'n');
insert into if_userinfo (no, name, age, sex, sns) values (2,'second',22,'m','y');
...
when job!='CLERK' and job!='SALESMAN' then '사원'
end`job2` from emp;
-- +--------+-----------+----------+
-- | ENAME  | JOB       | JOB2     |
-- +--------+-----------+----------+
