...
select b.deptno, b.dname, avg(a.sal)`AVG_SAL`, max(a.sal)`MAX_SAL`, min(a.sal)`MIN_SAL`, count(*)`cnt`
from emp a , dept b
...
select deptno, dname, avg(a.sal)`AVG_SAL`, max(a.sal)`MAX_SAL`, min(a.sal)`MIN_SAL`, count(*)`cnt`
from emp a natural join dept b
...
from emp a, dept b
where a.deptno = b.deptno and sal > (select avg(sal) from emp where a.deptno = 20)
...
select max(sal) from emp where deptno group by deptno;

select *
from emp
where sal = any(select max(sal) from emp where deptno group by deptno);

select *
from emp
where sal = some(select max(sal) from emp where deptno group by deptno);
