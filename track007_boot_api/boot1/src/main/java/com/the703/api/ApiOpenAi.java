...
public class ApiOpenAi {
    ...
    public String getAiResponse(String userMessage) {
    	...
    	// user -> 당신은 업로드된 문서 내용을 기반으로 답변하는 전문 비서입니다. (역할 부여)
    	// content -> ~~ 내용을 이모티콘으로 요약해줘
    	
    	try {
    		
	    	// 2. RestClient 스타일 세팅하여 값 받아오기
    		String responseBody = restClient.post()
    							.contentType(MediaType.APPLICATION_JSON)
    							.header("Authorization", "Bearer " + apiKey)
    							.body(body)
    							.retrieve()
    							.body(String.class); // 응답을 String으로 받아옴
    		...
    		throw new RuntimeException("open ai 호출 응답 파싱오류", e);
    	}
    }
